...
Register<bit<32>, _>(6) circular_queue;
...
control SwitchIngress(
    ...
    apply {
        if (hdr.ipv4.isValid()) {
            if(hdr.ipv4.dst_addr == 0x0a320064){
                ig_md.iterator = update_iterator.execute(0);
                ig_md.next_hop = read_queue.execute(ig_md.iterator);
                if ( ig_md.next_hop == 1) ig_md.available_server_meta = 0x0a320006;
                else ig_md.available_server_meta = 0x0a320010;
                LB.apply(); 
            }
            ipv4_lpm.apply();
